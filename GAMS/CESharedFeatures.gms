...
*Meet demand and reserves
         ...
         limitshiftingdemandupper(h)          limit the amount of demand that can be shifted
         limitshiftingdemandlower(h)          limit the amount of demand that can be shifted
         meetshiftingdemand(h)
*         meetshiftingdemandupper(h)
*         meetshiftingdemandlower(h)
...
meetdemand(h)..          sum(tech,vGentech(tech,h))+sum(egu,vGen(egu,h)) =g= ((pDemand(h)+vShiftedDemand(h))
                                                        + sum(storageegu,vCharge(storageegu,h)) + sum(storagetech,vChargetech(storagetech,h)));

limitshiftingdemandupper(h)..       vShiftedDemand(h) =l= pDemand(h)*pDemandShifter;
limitshiftingdemandlower(h)..       vShiftedDemand(h) =g= -1*pDemand(h)*pDemandShifter;
meetshiftingdemand(h)$(mod(ord(h),pDemandShiftingBlock)=1).. sum(hh$((ord(hh)>=ord(h) and (ord(hh)<=ord(h)+pDemandShiftingBlock-1))), vShiftedDemand(hh))=e= 0;
*meetshiftingdemandupper(h)..        vShiftedDemand(h)+vShiftedDemand(h++1)+vShiftedDemand(h++2)+vShiftedDemand(h++3)=e=0;
*meetshiftingdemandlower(h)..       vShiftedDemand(h)+vShiftedDemand(h--1)+vShiftedDemand(h--2)+vShiftedDemand(h--3)=e=0;
